...
public class ThirdExercise extends AppCompatActivity {
    ...
    private void logNoseLandmark(FaceMeshResult result, boolean showPixelValues) {
        ...
        if(start) {
            ...
            if (depth - 7 > Depth && !FlagSecondCheckR) {
                Log.v("eaf", IrisDiameterRight + "     " + irisDiameterRightNew);
                if (IrisDiameterRight + 5 > irisDiameterRightNew
                        && IrisDiameterRight - 5 < irisDiameterRightNew) {
                    Log.v("eaf", "PRAVO");
            ...
            if (depth + 13 < Depth && !FlagSecondCheckL) {
                Log.v("eaf", IrisDiameterLeft + "     " + irisDiameterLeftNew);
                if (IrisDiameterLeft + 5 > irisDiameterLeftNew
                        && IrisDiameterLeft - 5 < irisDiameterLeftNew) {
                    Log.v("eaf", "LEVO");
            ...
            if(FlagFirstCheckR && FlagSecondCheckR && FlagFirstCheckL && FlagSecondCheckL){
                //   flag = false;
    ...
    private void ExComplete(){
        runOnUiThread(new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ed.putBoolean("ThirdComplete", true);
                ed.apply();
                AlertDialog.Builder a_builder = new AlertDialog.Builder(ThirdExercise.this);
